...
class MGSTCNCell(RNNCell):
    ...
    def __init__(self, num_units, adj_mx, k, num_nodes, ratelist=[],num_proj=None,
        ...
        supports = []
        for adj_mx2 in adj_mx:
            supports.append(self.calculate_scaled_laplacian(adj_mx2))
        ...
        with tf.variable_scope(scope or "mgstcn_cell2",reuse=tf.AUTO_REUSE):
            with tf.variable_scope("gates"):
                ...
                for i in range(0, len(self._supports)):
                    globals()["v_1" + str(i)]=(self._gconv(inputs,graph=self._supports[i],h=h,output_size= self._num_units))

                for i in range(0,len(self._supports)):
                    for j in range(0, len(self._rate)):
                        with tf.variable_scope(str(i)+"modular" + str(j)):
                            globals()[str(i)+"v_2" + str(j)]=self._gconv(inputs,graph=self._supports[i],h=globals()["ht" + str(j)], output_size=self._num_units)

                value = self._ModularGraphConv(inputs,h=h, output_size=(4+ len(self._rate))*self._num_units)
    ...
    def _gconv(self, inputs=None,graph=None,h=None,output_size=None,gname="gconv"):
        batch_size = inputs.get_shape()[0].value
        inputs = tf.reshape(inputs, (batch_size, self._num_nodes, -1))

        h = tf.reshape(h, (batch_size, self._num_nodes, -1))
        inputs_and_state = tf.concat([inputs, h], axis=2)
        ...
        with tf.variable_scope(scope):
            x1 = tf.sparse_tensor_dense_matmul(graph, x0)
            x = self._concat(x, x1)
            for k in range(2, self._max_diffusion_step + 1):
                x2 = 2 * tf.sparse_tensor_dense_matmul(graph, x1) - x0
                x = self._concat(x, x2)
                x1, x0 = x2, x1
            ...
            weights = tf.get_variable(gname+'weights', [input_size * num_matrices, output_size], dtype=dtype,initializer=tf.constant_initializer(1))
